  ...
  const fetchHandler = async () => {
    try {
      const { data } = await axios.get(
        "https://gadi-driver-u8ym.vercel.app/api/v1/location"
      );
      setData(data);
    } catch (e) {
      console.log(e);
    ...
    const postHandler = async (e) => {
      ...
      try {
        const { data } = await axios.post(
          "https://gadi-driver-u8ym.vercel.app/api/v1/locationn",
          {
            location,
          }
        );
        toast.success(`${data.location} is Created`);
        fetchHandler();
        props.onHide();
      } catch (e) {
        console.log(e);
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(
        `https://gadi-driver-u8ym.vercel.app/api/v1/locationn/${id}`
      );
    ...
    <>
      ...
      <section>
        ...
        >
          ...
          >
            All Location's ( Total : {data?.length} )
          </span>
          <button
            onClick={() => {
              setModalShow(true);
        ...
        <section className="sectionCont">
          {data?.length === 0 || !data ? (
            <InfoAlert message="No Location Found" />
          ) : (
            <>
