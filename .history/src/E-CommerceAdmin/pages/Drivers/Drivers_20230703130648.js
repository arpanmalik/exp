...
const EAdminDelivery = () => {
  ...
  const fetchData = useCallback(async() => {
    try {
      const { data } = await axios.get("https://gadi-driver-u8ym.vercel.app/api/v1/admin/Driver" , {
        headers : {
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(`https://gadi-driver-u8ym.vercel.app/api/v1/admin/64a004c541d5bbfc791b8552`)
                    ...
                    <Table>
                      ...
                      <tbody>
                        {slicedData?.map((i, index) => (
                          <tr key={index}>
                            <td> #{index + 1} </td>
                            <td> {EmptyData(i.firstName)} </td>
                            <td> {EmptyData(i.lastName)} </td>
                            <td> {EmptyData(i.gender)} </td>
                            <td> {EmptyData(i.licienceNumber)} </td>
                            <td> {EmptyData(i.email)} </td>
                            <td>
