...
const Language = () => {
  ...
  const fetchHandler = async () => {
    try{
      const { data  } = await axios.get("https://gadi-driver-u8ym.vercel.app/api/v1/languages")
      setData(data)
    }catch(e){
      console.log(e)
    }
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const postHandler = async (e) => {
      e.preventDefault()
      try{
        const { data } = await axios.post("https://gadi-driver-u8ym.vercel.app/api/v1/languages" , {language})
        toast.success(`${data.language } Created`)
        fetchHandler()
        props.onHide()
      }catch(e) { 
         console.log(e)
  ...
  const deleteHandler = async (id) => {
    try{
      const { data } = await axios.delete(`https://gadi-driver-u8ym.vercel.app/api/v1/languages/${id}`)
