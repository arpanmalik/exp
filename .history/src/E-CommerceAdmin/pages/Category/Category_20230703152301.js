...
const ECategory = () => {
  ...
  const [ modalShow2 , setModalShow2] 
  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get("https://gadi-driver-u8ym.vercel.app/api/v1/category");
      setData(data.data);
    } catch (e) {
      console.log(e);
    }
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const postHandler = async (e) => {
      e.preventDefault();
      try {
        const { data } = await axios.post("https://gadi-driver-u8ym.vercel.app/api/v1/category" , {category}) ;
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(`https://gadi-driver-u8ym.vercel.app/api/v1/category/${id}`);
